...
void Spi::end()
{
    ...
    PORT_SET_MUX(sin, PORT_MUX_DISABLED);
    PORT_SET_MUX(sout, PORT_MUX_DISABLED);
    PORT_SET_MUX(sck, PORT_MUX_DISABLED);
    SIM_HAL_DisableClock(SIM, gate_name);
}

uint32_t Spi::applySettings(SPISettings settings)
{
    uint32_t actual = DSPI_HAL_SetBaudRate(instance, kDspiCtar1, settings.clockFreq, clock);
    dspi_data_format_config_t config = {8, settings.polarity, settings.phase, settings.direction};
...
uint32_t Spi::beginTransaction(uint32_t chip_select)
{
    ...
    PORT_APPLY_MUX(cs); //change to set MUX with fixed mux#?
